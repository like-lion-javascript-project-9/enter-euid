export const createUserProfile = (src, alt, name, address) => {
  ...
  <div class="userInfoProfile flex flex-grow">
          <div class="userInfoInner aspect-square h-[40px] w-[40px] ml-3">
            <img
              src="/${src}"
              alt="${alt}"
...
export const createUserMannerTemperature = (temerature) => {
  ...
  return `
 <span
  class="userInfoManner__temperature text-sm font-bold text-secondary">${temerature}</span>
  <span class="userInfoManner__emoji">${emoji}</span> 
  <span class="userInfoManner__mannerTemperature ml-4 block text-xs text-[#919191]">매너온도</span>
    `;
};
